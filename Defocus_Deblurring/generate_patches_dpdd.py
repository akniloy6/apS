...
def filter_patch_sharpness(
    ...
    for i in range(num_of_img_patches):
        fitnessVal_3.append(
            shapness_measure(cv2.cvtColor(patches_trg_c_temp[i], cv2.COLOR_BGR2GRAY), 3)
        )
        fitnessVal_7.append(
            shapness_measure(cv2.cvtColor(patches_trg_c_temp[i], cv2.COLOR_BGR2GRAY), 7)
        )
        fitnessVal_11.append(
            shapness_measure(
                cv2.cvtColor(patches_trg_c_temp[i], cv2.COLOR_BGR2GRAY), 11
            )
        )
        fitnessVal_15.append(
            shapness_measure(
                cv2.cvtColor(patches_trg_c_temp[i], cv2.COLOR_BGR2GRAY), 15
            )
        )
    ...
    fitnessVal_3 = (fitnessVal_3 - np.min(fitnessVal_3)) / np.max(
        (fitnessVal_3 - np.min(fitnessVal_3))
    )
    fitnessVal_7 = (fitnessVal_7 - np.min(fitnessVal_7)) / np.max(
        (fitnessVal_7 - np.min(fitnessVal_7))
    )
    fitnessVal_11 = (fitnessVal_11 - np.min(fitnessVal_11)) / np.max(
        (fitnessVal_11 - np.min(fitnessVal_11))
    )
    fitnessVal_15 = (fitnessVal_15 - np.min(fitnessVal_15)) / np.max(
        (fitnessVal_15 - np.min(fitnessVal_15))
    )
...
def slice_stride(_img_src_c, _img_trg_c, _img_src_l, _img_src_r):
    ...
    for r in range(0, _img_src_c.shape[0], stride[0]):
        for c in range(0, _img_src_c.shape[1], stride[1]):
            if (r + patch_size[0]) <= _img_src_c.shape[0] and (
                c + patch_size[1]
            ) <= _img_src_c.shape[1]:
                patches_src_c_temp.append(
                    _img_src_c[r : r + patch_size[0], c : c + patch_size[1]]
                )
                patches_trg_c_temp.append(
                    _img_trg_c[r : r + patch_size[0], c : c + patch_size[1]]
                )
                patches_src_l_temp.append(
                    _img_src_l[r : r + patch_size[0], c : c + patch_size[1]]
                )
                patches_src_r_temp.append(
                    _img_src_r[r : r + patch_size[0], c : c + patch_size[1]]
                )
                ...
                [
                    r,
                    r + patch_size[0],
                    _img_src_c.shape[1] - patch_size[1],
                    _img_src_c.shape[1],
                ]
                in coordinates_list
            ):
                ...
                [
                    _img_src_c.shape[0] - patch_size[0],
                    _img_src_c.shape[0],
                    c,
                    c + patch_size[1],
                ]
                ...
                patches_src_c_temp.append(
                    _img_src_c[
                        _img_src_c.shape[0] - patch_size[0] : _img_src_c.shape[0],
                        c : c + patch_size[1],
                    ]
                )
                patches_trg_c_temp.append(
                    _img_trg_c[
                        _img_trg_c.shape[0] - patch_size[0] : _img_trg_c.shape[0],
                        c : c + patch_size[1],
                    ]
                )
                patches_src_l_temp.append(
                    _img_src_l[
                        _img_src_l.shape[0] - patch_size[0] : _img_src_l.shape[0],
                        c : c + patch_size[1],
                    ]
                )
                patches_src_r_temp.append(
                    _img_src_r[
                        _img_src_r.shape[0] - patch_size[0] : _img_src_r.shape[0],
                        c : c + patch_size[1],
                    ]
                )
                coordinates_list.append(
...
def train_files(file_):
    ...
    lrC_patches, hrC_patches, lrL_patches, lrR_patches = slice_stride(
        lrC_img, hrC_img, lrL_img, lrR_img
    )
    lrC_patches, hrC_patches, lrL_patches, lrR_patches = filter_patch_sharpness(
        lrC_patches, hrC_patches, lrL_patches, lrR_patches
    )
    num_patch = 0
    for lrC_patch, hrC_patch, lrL_patch, lrR_patch in zip(
        lrC_patches, hrC_patches, lrL_patches, lrR_patches
    ):
        num_patch += 1
