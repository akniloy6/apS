...
parser.add_argument(
    "--input_dir",
    default="./Datasets/test/DPDD/",
    type=str,
    help="Directory of validation images",
)
parser.add_argument(
    "--result_dir",
    default="./results/Defocus_Deblurring/",
    type=str,
    help="Directory for results",
)
parser.add_argument(
    "--weights",
    default="./pretrained_models/dual_pixel_defocus_deblurring.pth",
    type=str,
    help="Path to weights",
)
parser.add_argument(
    "--save_images", action="store_true", help="Save Sharp images in result directory"
)
...
with torch.inference_mode():
    for fileL, fileR, fileC in tqdm(zip(filesL, filesR, filesC), total=len(filesC)):
        imgL = np.float32(utils.load_img16(fileL)) / 65535.0
        imgR = np.float32(utils.load_img16(fileR)) / 65535.0
        imgC = np.float32(utils.load_img16(fileC)) / 65535.0
...
psnr_indoor, mae_indoor, ssim_indoor, pips_indoor = (
    psnr[indoor_labels - 1],
    mae[indoor_labels - 1],
    ssim[indoor_labels - 1],
    pips[indoor_labels - 1],
)
psnr_outdoor, mae_outdoor, ssim_outdoor, pips_outdoor = (
    psnr[outdoor_labels - 1],
    mae[outdoor_labels - 1],
    ssim[outdoor_labels - 1],
    pips[outdoor_labels - 1],
)

print(
    "Overall: PSNR {:4f} SSIM {:4f} MAE {:4f} LPIPS {:4f}".format(
        np.mean(psnr), np.mean(ssim), np.mean(mae), np.mean(pips)
    )
)
print(
    "Indoor:  PSNR {:4f} SSIM {:4f} MAE {:4f} LPIPS {:4f}".format(
        np.mean(psnr_indoor),
        np.mean(ssim_indoor),
        np.mean(mae_indoor),
        np.mean(pips_indoor),
...
print(
    "Outdoor: PSNR {:4f} SSIM {:4f} MAE {:4f} LPIPS {:4f}".format(
        np.mean(psnr_outdoor),
        np.mean(ssim_outdoor),
        np.mean(mae_outdoor),
        np.mean(pips_outdoor),
    )
)
